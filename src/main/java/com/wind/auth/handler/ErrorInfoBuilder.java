...
public class ErrorInfoBuilder implements HandlerExceptionResolver, Ordered {
     ...
     * 构建错误信息.(ErrorInfo)
     */
    private ErrorInfo getErrorInfo(HttpServletRequest request, Throwable error) {
        ...
        errorInfo.setStackTrace(getStackTraceInfo(error, true));
        // errorInfo.setStackTrace(getStackTraceInfo(error, isIncludeStackTrace(request)));
        return errorInfo;
    }

    /**
     * 获取错误.(Error/Exception) //@see DefaultErrorAttributes #addErrorDetails
     */
    ...
    /**
     * 获取通信状态(HttpStatus) //@see AbstractErrorController #getStatus
     */
    ...
    /**
     * 获取堆栈轨迹(StackTrace) //@see DefaultErrorAttributes #addStackTrace
     */
    ...
    /**
     * 判断是否包含堆栈轨迹.(isIncludeStackTrace) //@see BasicErrorController #isIncludeStackTrace
     */
    ...
    /**
     * 保存错误/异常. //@see DispatcherServlet #processHandlerException 进行选举HandlerExceptionResolver
     */
    @Override
